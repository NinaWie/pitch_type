...
if PATH is not "concat":
    ...
    data_raw = np.load("cf_all_coord.npy") #("interpolated.npy")
    print("data loaded")
else:
...
# print("classe trained on:", pitchi[2])
# prepro.set_labels(pitchi[2])  # change_restore
...
print(data_raw.shape, labels.shape, labels)


def runscript(data_in, labels_string, BATCH_SZ=40, EPOCHS = 60, batch_nr_in_epoch = 100, align = False,
    ...
    for epoch in range(EPOCHS + 1):
        ...
        print(loss_test, acc_train[-1], acc_test[-1], Tools.balanced_accuracy(pitches_test, labels_string_test))
...
#filter_sizes = [56, 128, 256]
#kernel_sizes = [3, 5, 9]
#hidden_ff = [0, 56, 128]

# alignd = [True, False]
# regular = [0, 0.0005, 0.001]
# hidden_ff = [0, 128, 256]
# kernel_sizes = [5, 9, 11]
#
# for hi1 in hidden_ff:
#     for ke1 in kernel_sizes:
#         for ke2 in kernel_sizes:
#             for re in regular:
#                 for al in align:
#                    runscript(data_raw, labels, first_hidden_dense= hi1, first_conv_kernel = ke1, second_conv_kernel = ke2, regularization=re, align = al)

ind = [tf.nn.relu, 0.0, False, 0.0005, 0.0005, 0, 3, 1024, 9, 0, 256] #[tf.nn.relu, 0.6, True, 0.00025, 0.0005, 0, 11, 512, 5, 256, 0]
print(ind)

runscript(data_raw, labels, EPOCHS = 60, act = ind[0], rate_dropout=ind[1], align= ind[2], learning_rate=ind[3],
regularization=ind[4], first_conv_filters=ind[5], first_conv_kernel=int(ind[6]), second_conv_filter=ind[7],
second_conv_kernel=int(ind[8]), first_hidden_dense=ind[9], second_hidden_dense=ind[10], network = "adjustable conv1d")
