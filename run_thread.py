...
class Runner(threading.Thread):
    ...
    def __init__(self, data, labels_string, unique, normalize = False, SAVE = None, RESTORE = None, BATCH_SZ=40, EPOCHS = 60, batch_nr_in_epoch = 100, align = False,
    ...
    def run(self):
        self.labels_string = np.array(self.labels_string)
        print(self.SAVE)
        try:
            self.unique = self.unique.tolist()
        ...
        if self.align:
            self.data = Tools.align_frames(self.data, prepro.get_release_frame(60, 120), 60, 40)
        ...
        if self.RESTORE is not None:
            ...
            saver.self.RESTORE(sess, self.RESTORE)
            out_test = sess.run(out, {x: self.data, training: False})
            # pitches_test = Tools.decode_one_hot(out_test, self.unique)
            # acc = Tools.accuracy(pitches_test, self.labels_string)
            # print("True                Test                 ", self.unique)
            # for i in range(len(pitches_test)):
            #     print('{:20}'.format(self.labels_string[i]), '{:20}'.format(pitches_test[i]), ['%.2f        ' % elem for elem in out_test[i]])
            return out_test
        # NET
        else:
            M,N,nr_joints,nr_coordinates = self.data.shape
            ...
            # Run session for EPOCH self.EPOCHS
            ...
            # # print(len(columns))
            # # print("Written to csv: ", BATCHSIZE, CUT_OFF_Classes , self.EPOCHS, self.act, self.align, self.batch_nr_in_epoch, self.rate_dropout
            # #                      , PATH, max(acc_test), self.learning_rate, len_train, losses, self.n_hidden, self.nr_layers, self.network, nr_classes,
            # #                      nr_joints, acc_train, acc_test)
            #
            # add = pd.self.dataFrame([[0, BATCHSIZE, CUT_OFF_Classes , self.EPOCHS, self.act, self.align, self.batch_nr_in_epoch, self.rate_dropout
            #                      , PATH, max(acc_test), self.learning_rate, len_train, losses, self.n_hidden, self.nr_layers, self.network, nr_classes,
            ...
            if self.SAVE is not None:
                print(self.SAVE)
                saver.self.SAVE(sess, self.SAVE)
                print("model saved")
...
#     for le in b:
#         for ac in c:
#runscript(self.data_raw, labels, self.network="RNN_network_tflearn")
