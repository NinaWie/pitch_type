...
def plot_trajectory_on_video(vid_path, out_path, ball_trajectory):
    ball_trajectory[:,2]-=11
    radius = 10
...
def detect_ball(folder, joints_array=None, min_area = 400, plotting=True, min_length_first=5, every_x_frame=1, roi=None, refine = False):
    """
    ...
    while True:
        ...
        # SHIFTED CANDIDATES:
            ...
            if len(ankle_move)>=min_length_first and t-ankle_move[-min_length_first]<10: #len(ankle_move)==3:
                ...
                if refine:
                    RADIUS = 10 ### ersetzen durch config ding!
                    RADIUS_LOWER = min(RADIUS, first_move_frame)
                    range_joints = joints_array[first_move_frame - RADIUS_LOWER: first_move_frame + RADIUS]
