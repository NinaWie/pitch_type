...
#           coordinates_pitcher[key]=coordinates_pitcher2.tolist()
    ...
    try:
        ...
        label = (cf["Pitch Type"].values)[location_play]
        pitcher_arr = []
        for i in range(datContent['pitch_frame_index']):
            pitcher_arr.append(eval(df_res.values[i].replace('\n', ',').replace('[  ', '[').replace("  ", ",").replace(",,",",")))
        pitches, acc = runner.run(pitcher_array, label, all_classes, RESTORE="./model")
