        ...
        if(nA != 0 and nB != 0):
            ...
            for i in range(nA):
                for j in range(nB):
                    ...
                    score_midpts = np.ply(vec_x, vec[0]) + np.multiply(vec_y, vec[1])
...
if __name__ == "__main__":
    ...
    path_to_video = "new.mp4"
    out_dir = "ice_outputs/"
    number_frames_to_process = 50
    ...
    t=0
    dic = {}
    while t<number_frames_to_process: # if whole video, just do while True
        # Capture frame-by-frame
        ret, frame = video_capture.read()
        x,y, _ = frame.shape
        impo = frame[:int(0.5*x), int(0.3*y):int(0.8*y)] # relevant part of the image
        try:
            canvas, coordinates = handle_one(impo)
        except ZeroDivisionError: # I don't know why this occurs
            continue
        dic["frame "+str(t)] = coordinates.tolist()
        # Display the resulting frame
        # cv2.imshow('Video', canvas)
        t+=1
        print(t)
        cv2.imwrite(out_dir+str(t)+".jpg",canvas)
        if cv2.waitKey(1) & 0xFF == ord('q'):
            break

    with open("ice_outputs/coordinate_output.json", "w") as outfile:
