...
def handle_one(oriImg, colour_image=False):
    tic = time.time()
 #   print 1
...
if __name__ == "__main__":
    ...
    parser.add_argument('input_file', default = "new.mp4", metavar='DIR', # Video file to be processed
    ...
    t=0
    dic = {}
    while t<int(args.number_frames):
        ...
        t+=1
        cv2.imwrite(os.path.join(out_dir, "Frame"+str(t)+".jpg"),canvas)
        if cv2.waitKey(1) & 0xFF == ord('q'):
            break

    with open(os.path.join(out_dir, "coordinates.json"), "w") as outfile:
