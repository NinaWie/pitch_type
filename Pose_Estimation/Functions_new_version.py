...
def player_localization(handle_one ,old_array):
    ...
    for i in range(np.asarray(handle_one).shape[0]):
        ...
        zerrow1=np.where(np.asarray(handle_one)[i,:,0]!=0)[0]
    ...
    if not np.any(np.array(intersections)>0.1):
        print("missing frame", "with intersections:", intersections) #, "with players", df[player][frame])
        res = [[0,0] for i in range(18)]
    else:
        if np.sum(np.array(intersections)>0.5)>1:
            print("too many overlaping", "with intersections:", intersections) #, "with players", df[player][frame])
            res=[[0,0] for i in range(18)]
        else:
            res = handle_one[np.argmax(intersections)]

    array_stored=np.asarray(res)
    #array_stored[np.where(array_stored==0)]=old_array[np.where(array_stored==0)]
    old_array=array_stored
    return res, old_array
...
def mix_right_left(df):
    ...
    for index in index_list:
        for i in range(1, len(df)-1):
            ...
            # cond1: if distance between right joint last frame and this frame is smaller than right joint current frame and left joint current frame
            cond1 = abs(zeros_filled[i, r, 1] - zeros_filled[i-1, r, 1]) + abs(zeros_filled[i, r, 0] - zeros_filled[i-1, r, 0]) > abs(zeros_filled[i, r, 1] - zeros_filled[i-1, l, 1])+ abs(zeros_filled[i, r, 0] - zeros_filled[i-1, l, 0])
            cond2 = abs(zeros_filled[i, l, 1] - zeros_filled[i-1, l, 1]) + abs(zeros_filled[i, l, 0] - zeros_filled[i-1, l, 0]) > abs(zeros_filled[i-1, r, 1] - zeros_filled[i, l, 1])+ abs(zeros_filled[i-1 , r, 0] - zeros_filled[i, l, 0])
            # # FOR EUCLEDEAN INSTEAD OF CHESSBORD DISTANCE
            ...
            # DOES NOT WORK PROPERLY; WRITTEN IN ORDER TO AVOID RIGHT KNEE MOVING TO LEFT KNEE POSITION
            # elif cond1:
            #     df[i,r, 0] = 0
            #     df[i,r, 1] = 0
            #     zeros_filled[i,r] = zeros_filled[i-1, r]
...
def df_coordinates(df,centerd, player_list, interpolate = True, smooth = True, fps = 20):

    if interpolate:
        new_df = interpolate(df)
